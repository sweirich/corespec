...
with tm : Set :=  (*r types and kinds *)
 ...
 | a_Pattern2 (a1:tm) (a2:tm) (b1:tm) (b2:tm)
 | a_DataCon (K:datacon)
...
with open_tm_wrt_co_rec (k:nat) (g5:co) (a_5:tm) {struct a_5}: tm :=
  match a_5 with
  | a_Star => a_Star 
  ...
  | (a_App a nu5 b) => a_App (open_tm_wrt_co_rec k g5 a) nu5 (open_tm_wrt_co_rec k g5 b)
  | (a_Fam F) => a_Fam F
  ...
  | (a_Pattern2 a1 a2 b1 b2) => a_Pattern2 (open_tm_wrt_co_rec k g5 a1) (open_tm_wrt_co_rec k g5 a2) (open_tm_wrt_co_rec k g5 b1) (open_tm_wrt_co_rec k g5 b2)
...
with open_tm_wrt_tm_rec (k:nat) (a_5:tm) (a__6:tm) {struct a__6}: tm :=
  match a__6 with
  | a_Star => a_Star 
  | (a_Var_b nat) => if (k === nat) then a_5 else (a_Var_b nat)
  | (a_Var_f x) => a_Var_f x
  | (a_Abs rho A b) => a_Abs rho (open_tm_wrt_tm_rec k a_5 A) (open_tm_wrt_tm_rec (S k) a_5 b)
  | (a_UAbs rho b) => a_UAbs rho (open_tm_wrt_tm_rec (S k) a_5 b)
  | (a_App a nu5 b) => a_App (open_tm_wrt_tm_rec k a_5 a) nu5 (open_tm_wrt_tm_rec k a_5 b)
  | (a_Fam F) => a_Fam F
  | (a_Pi rho A B) => a_Pi rho (open_tm_wrt_tm_rec k a_5 A) (open_tm_wrt_tm_rec (S k) a_5 B)
  | (a_Conv a R g) => a_Conv (open_tm_wrt_tm_rec k a_5 a) R (open_co_wrt_tm_rec k a_5 g)
  | (a_CPi phi B) => a_CPi (open_constraint_wrt_tm_rec k a_5 phi) (open_tm_wrt_tm_rec k a_5 B)
  | (a_CAbs phi b) => a_CAbs (open_constraint_wrt_tm_rec k a_5 phi) (open_tm_wrt_tm_rec k a_5 b)
  | (a_UCAbs b) => a_UCAbs (open_tm_wrt_tm_rec k a_5 b)
  | (a_CApp a g) => a_CApp (open_tm_wrt_tm_rec k a_5 a) (open_co_wrt_tm_rec k a_5 g)
  | a_Bullet => a_Bullet 
  | (a_Pattern R a' a b1 b2) => a_Pattern R (open_tm_wrt_tm_rec k a_5 a') (open_tm_wrt_tm_rec k a_5 a) (open_tm_wrt_tm_rec k a_5 b1) (open_tm_wrt_tm_rec k a_5 b2)
  | (a_Pattern2 a1 a2 b1 b2) => a_Pattern2 (open_tm_wrt_tm_rec k a_5 a1) (open_tm_wrt_tm_rec k a_5 a2) (open_tm_wrt_tm_rec k a_5 b1) (open_tm_wrt_tm_rec k a_5 b2)
  | (a_DataCon K) => a_DataCon K
  | (a_Case a brs5) => a_Case (open_tm_wrt_tm_rec k a_5 a) (open_brs_wrt_tm_rec k a_5 brs5)
  | (a_Sub R a) => a_Sub R (open_tm_wrt_tm_rec k a_5 a)
end
...
Definition open_sig_sort_wrt_co_rec (k:nat) (g5:co) (sig_sort5:sig_sort) : sig_sort :=
  ...
  | (Ax a A R Rs) => Ax (open_tm_wrt_co_rec k g5 a) (open_tm_wrt_co_rec k g5 A) R Rs
end.

Definition open_sig_sort_wrt_tm_rec (k:nat) (a5:tm) (sig_sort5:sig_sort) : sig_sort :=
  match sig_sort5 with
  | (Cs A Rs) => Cs (open_tm_wrt_tm_rec k a5 A) Rs
  | (Ax a A R Rs) => Ax (open_tm_wrt_tm_rec k a5 a) (open_tm_wrt_tm_rec k a5 A) R Rs
end.
...
Definition open_tm_wrt_co g5 a_5 := open_tm_wrt_co_rec 0 a_5 g5.
...
Definition open_tm_wrt_tm a_5 a__6 := open_tm_wrt_tm_rec 0 a__6 a_5.
...
with lc_tm : tm -> Prop :=    (* defn lc_tm *)
 ...
 | lc_a_UAbs : forall (L:vars) (rho:relflag) (b:tm),
      ( forall x , x \notin  L  -> lc_tm  ( open_tm_wrt_tm b (a_Var_f x) )  )  ->
     (lc_tm (a_UAbs rho b))
 | lc_a_App : forall (a:tm) (nu5:nu) (b:tm),
     (lc_tm a) ->
     (lc_tm b) ->
     (lc_tm (a_App a nu5 b))
 ...
 | lc_a_Pattern2 : forall (a1 a2 b1 b2:tm),
     (lc_tm a1) ->
     (lc_tm a2) ->
     (lc_tm b1) ->
     (lc_tm b2) ->
     (lc_tm (a_Pattern2 a1 a2 b1 b2))
...
Inductive lc_sig_sort : sig_sort -> Prop :=    (* defn lc_sig_sort *)
 | lc_Cs : forall (A:tm) (Rs:roles),
     (lc_tm A) ->
     (lc_sig_sort (Cs A Rs))
 | lc_Ax : forall (a A:tm) (R:role) (Rs:roles),
     (lc_tm a) ->
     (lc_tm A) ->
     (lc_sig_sort (Ax a A R Rs)).
(** free variables *)
...
with fv_tm_tm_tm (a_5:tm) : vars :=
  ...
  | (a_App a nu5 b) => (fv_tm_tm_tm a) \u (fv_tm_tm_tm b)
  | (a_Fam F) => {}
  ...
  | (a_Pattern2 a1 a2 b1 b2) => (fv_tm_tm_tm a1) \u (fv_tm_tm_tm a2) \u (fv_tm_tm_tm b1) \u (fv_tm_tm_tm b2)
  | (a_DataCon K) => {}
...
with fv_co_co_tm (a_5:tm) : vars :=
  ...
  | (a_App a nu5 b) => (fv_co_co_tm a) \u (fv_co_co_tm b)
  | (a_Fam F) => {}
  ...
  | (a_Pattern2 a1 a2 b1 b2) => (fv_co_co_tm a1) \u (fv_co_co_tm a2) \u (fv_co_co_tm b1) \u (fv_co_co_tm b2)
  | (a_DataCon K) => {}
...
Definition fv_tm_tm_sig_sort (sig_sort5:sig_sort) : vars :=
  ...
  | (Ax a A R Rs) => (fv_tm_tm_tm a) \u (fv_tm_tm_tm A)
end.

Definition fv_co_co_sig_sort (sig_sort5:sig_sort) : vars :=
  match sig_sort5 with
  | (Cs A Rs) => (fv_co_co_tm A)
  | (Ax a A R Rs) => (fv_co_co_tm a) \u (fv_co_co_tm A)
end.
...
with tm_subst_tm_tm (a_5:tm) (x5:tmvar) (a__6:tm) {struct a__6} : tm :=
  match a__6 with
  | a_Star => a_Star 
  | (a_Var_b nat) => a_Var_b nat
  | (a_Var_f x) => (if eq_var x x5 then a_5 else (a_Var_f x))
  | (a_Abs rho A b) => a_Abs rho (tm_subst_tm_tm a_5 x5 A) (tm_subst_tm_tm a_5 x5 b)
  | (a_UAbs rho b) => a_UAbs rho (tm_subst_tm_tm a_5 x5 b)
  | (a_App a nu5 b) => a_App (tm_subst_tm_tm a_5 x5 a) nu5 (tm_subst_tm_tm a_5 x5 b)
  | (a_Fam F) => a_Fam F
  | (a_Pi rho A B) => a_Pi rho (tm_subst_tm_tm a_5 x5 A) (tm_subst_tm_tm a_5 x5 B)
  | (a_Conv a R g) => a_Conv (tm_subst_tm_tm a_5 x5 a) R (tm_subst_tm_co a_5 x5 g)
  | (a_CPi phi B) => a_CPi (tm_subst_tm_constraint a_5 x5 phi) (tm_subst_tm_tm a_5 x5 B)
  | (a_CAbs phi b) => a_CAbs (tm_subst_tm_constraint a_5 x5 phi) (tm_subst_tm_tm a_5 x5 b)
  | (a_UCAbs b) => a_UCAbs (tm_subst_tm_tm a_5 x5 b)
  | (a_CApp a g) => a_CApp (tm_subst_tm_tm a_5 x5 a) (tm_subst_tm_co a_5 x5 g)
  | a_Bullet => a_Bullet 
  | (a_Pattern R a' a b1 b2) => a_Pattern R (tm_subst_tm_tm a_5 x5 a') (tm_subst_tm_tm a_5 x5 a) (tm_subst_tm_tm a_5 x5 b1) (tm_subst_tm_tm a_5 x5 b2)
  | (a_Pattern2 a1 a2 b1 b2) => a_Pattern2 (tm_subst_tm_tm a_5 x5 a1) (tm_subst_tm_tm a_5 x5 a2) (tm_subst_tm_tm a_5 x5 b1) (tm_subst_tm_tm a_5 x5 b2)
  | (a_DataCon K) => a_DataCon K
  | (a_Case a brs5) => a_Case (tm_subst_tm_tm a_5 x5 a) (tm_subst_tm_brs a_5 x5 brs5)
  | (a_Sub R a) => a_Sub R (tm_subst_tm_tm a_5 x5 a)
end
...
with co_subst_co_tm (g5:co) (c5:covar) (a_5:tm) {struct a_5} : tm :=
  match a_5 with
  | a_Star => a_Star 
  | (a_Var_b nat) => a_Var_b nat
  | (a_Var_f x) => a_Var_f x
  ...
  | (a_App a nu5 b) => a_App (co_subst_co_tm g5 c5 a) nu5 (co_subst_co_tm g5 c5 b)
  | (a_Fam F) => a_Fam F
  ...
  | (a_Pattern2 a1 a2 b1 b2) => a_Pattern2 (co_subst_co_tm g5 c5 a1) (co_subst_co_tm g5 c5 a2) (co_subst_co_tm g5 c5 b1) (co_subst_co_tm g5 c5 b2)
  | (a_DataCon K) => a_DataCon K
...
Definition tm_subst_tm_sig_sort (a5:tm) (x5:tmvar) (sig_sort5:sig_sort) : sig_sort :=
  ...
  | (Ax a A R Rs) => Ax (tm_subst_tm_tm a5 x5 a) (tm_subst_tm_tm a5 x5 A) R Rs
end.

Definition co_subst_co_sig_sort (g5:co) (c5:covar) (sig_sort5:sig_sort) : sig_sort :=
  match sig_sort5 with
  | (Cs A Rs) => Cs (co_subst_co_tm g5 c5 A) Rs
  | (Ax a A R Rs) => Ax (co_subst_co_tm g5 c5 a) (co_subst_co_tm g5 c5 A) R Rs
end.


Definition app_role (rr : nu) : role :=
  match rr with
  | Rho _ => Nom
  | Role r => r 
  end.
...
Fixpoint erase_tm (a : tm) (r : role) : tm :=
   ...
   | a_TApp a R b => a_App (erase_tm a r) R (erase_tm b R)
   | a_App a Rel b => a_App (erase_tm a r) Rel (erase_tm b R)
   | a_App a Irrel b => a_App (erase_tm a r) Irrel a_Bullet
   | a_Fam F => a_Fam F
...
Definition erase_csort s r :=
 ...
 | Ax a A R Rs => Ax (erase_tm a r) (erase_tm A r) R Rs
end.
...
(* defns JPath *)
Inductive Path : const -> tm -> role -> roles -> Prop :=    (* defn Path *)
 | Path_AbsConst : forall (F:const) (R:role) (Rs:roles) (A:tm),
      binds  F  ( (Cs A Rs) )   toplevel   ->
     Path F (a_Fam F) R Rs
 | Path_Const : forall (F:const) (R:role) (Rs:roles) (a A:tm) (R1:role),
      binds  F  ( (Ax a A R1 Rs) )   toplevel   ->
      not (  ( SubRole R1 R )  )  ->
     Path F (a_Fam F) R  ( R :: Rs ) 
 | Path_App : forall (F:const) (a:tm) (nu5:nu) (b':tm) (R:role) (Rs:roles) (R1:role),
     lc_tm b' ->
     Path F a R  ( R1 :: Rs )  ->
      (  (app_role  nu5 )   =  R1 )  ->
     Path F  ( (a_App a nu5 b') )  R Rs
 | Path_CApp : forall (F:const) (a:tm) (R:role) (Rs:roles),
     Path F a R Rs ->
     Path F  ( (a_CApp a g_Triv) )  R Rs.

(* defns JPat *)
Inductive Pat : context -> tm -> tm -> role -> roles -> Prop :=    (* defn Pat *)
 | Pat_AbsConst : forall (F:const) (A:tm) (R:role) (Rs:roles),
      binds  F  ( (Cs A Rs) )   toplevel   ->
     Pat  nil  (a_Fam F) A R Rs
 | Pat_Const : forall (F:const) (A:tm) (R:role) (Rs:roles) (a:tm) (R1:role),
      binds  F  ( (Ax a A R1 Rs) )   toplevel   ->
      not (  ( SubRole R1 R )  )  ->
     Pat  nil  (a_Fam F) A R Rs
 | Pat_App : forall (L:vars) (G:context) (x:tmvar) (A1 a:tm) (nu5:nu) (B:tm) (R:role) (Rs:roles) (rho:relflag) (B1:tm) (R1:role),
     Pat G a (a_Pi rho A1 B1) R  ( R1 :: Rs )  ->
      ( forall y , y \notin  L  ->  tm_subst_tm_tm (a_Var_f  y )  x   B  =   ( open_tm_wrt_tm B1 (a_Var_f y) )   )  ->
      (  (app_role  nu5 )   =  R1 )  ->
     Pat  (( x ~ Tm  A1 ) ++  G )   ( (a_App a nu5 (a_Var_f x)) )  B R Rs
 | Pat_CApp : forall (L:vars) (G:context) (c:covar) (phi:constraint) (a B:tm) (R:role) (Rs:roles) (B1:tm),
     Pat G a (a_CPi phi B1) R Rs ->
      ( forall c1 , c1 \notin  L  ->  co_subst_co_tm (g_Var_f  c1 )  c   B  =   ( open_tm_wrt_co B1 (g_Var_f c1) )   )  ->
     Pat  (( c ~ Co  phi ) ++  G )   ( (a_CApp a g_Triv) )  B R Rs.
...
Inductive roleing : role_context -> tm -> role -> Prop :=    (* defn roleing *)
 ...
 | role_a_App : forall (W:role_context) (a:tm) (nu5:nu) (b:tm) (R:role),
     roleing W a R ->
     roleing W b  (app_role  nu5 )  ->
     roleing W  ( (a_App a nu5 b) )  R
 ...
 | role_a_Const : forall (W:role_context) (F:const) (R:role) (A:tm) (Rs:roles),
      ...
      binds  F  ( (Cs A Rs) )   toplevel   ->
     roleing W (a_Fam F) R
 | role_a_Fam : forall (W:role_context) (F:const) (R1:role) (a A:tm) (R:role) (Rs:roles),
      uniq  W  ->
      binds  F  ( (Ax a A R Rs) )   toplevel   ->
...
Inductive Par : role_context -> tm -> tm -> role -> Prop :=    (* defn Par *)
 | Par_Refl : forall (W:role_context) (a:tm) (R:role),
     roleing W a R ->
     Par W a a R
 | Par_Beta : forall (W:role_context) (a:tm) (nu5:nu) (b a' b':tm) (R:role) (rho:relflag),
     Par W a  ( (a_UAbs rho a') )  R ->
     Par W b b'  (app_role  nu5 )  ->
     Par W (a_App a nu5 b)  (open_tm_wrt_tm  a'   b' )  R
 | Par_App : forall (W:role_context) (a:tm) (nu5:nu) (b a' b':tm) (R:role),
     Par W a a' R ->
     Par W b b'  (app_role  nu5 )  ->
     Par W (a_App a nu5 b) (a_App a' nu5 b') R
 ...
 | Par_Axiom : forall (W:role_context) (F:const) (a:tm) (R:role) (A:tm) (R1:role) (Rs:roles),
      binds  F  ( (Ax a A R1 Rs) )   toplevel   ->
     SubRole R1 R ->
      uniq  W  ->
     Par W (a_Fam F) a R
 ...
 | Par_PatternTrue : forall (W:role_context) (R:role) (F:const) (a b1 b2 b1':tm) (R0:role) (a' b2':tm) (Rs:roles),
     Par W a a' R ->
     Par W b1 b1' R0 ->
     Par W b2 b2' R0 ->
     Path F a' R Rs ->
     Par W (a_Pattern R (a_Fam F) a b1 b2) b1' R0
 | Par_PatternFalse : forall (W:role_context) (R:role) (F:const) (a b1 b2 b2':tm) (R0:role) (sig_sort5:sig_sort) (a' b1':tm) (Rs:roles),
      binds  F  ( sig_sort5 )   toplevel   ->
     Par W a a' R ->
     Par W b1 b1' R0 ->
     Par W b2 b2' R0 ->
...
Inductive Beta : tm -> tm -> role -> Prop :=    (* defn Beta *)
 | Beta_AppAbs : forall (rho:relflag) (v:tm) (nu5:nu) (b:tm) (R1:role),
     lc_tm b ->
     Value R1  ( (a_UAbs rho v) )  ->
     Beta (a_App  ( (a_UAbs rho v) )  nu5 b)  (open_tm_wrt_tm  v   b )  R1
 | Beta_CAppCAbs : forall (a':tm) (R:role),
     lc_tm (a_UCAbs a') ->
     Beta (a_CApp  ( (a_UCAbs a') )  g_Triv)  (open_tm_wrt_co  a'   g_Triv )  R
 | Beta_Axiom : forall (F:const) (a:tm) (R1:role) (A:tm) (R:role) (Rs:roles),
      binds  F  ( (Ax a A R Rs) )   toplevel   ->
     SubRole R R1 ->
     Beta (a_Fam F) a R1
 | Beta_PatternTrue : forall (R:role) (F:const) (a b1 b2:tm) (R0:role) (Rs:roles),
     lc_tm b2 ->
     lc_tm b1 ->
     Path F a R Rs ->
     Beta (a_Pattern R (a_Fam F) a b1 b2) b1 R0
 | Beta_PatternFalse : forall (R:role) (F:const) (a b1 b2:tm) (R0:role) (sig_sort5:sig_sort) (Rs:roles),
     lc_tm b1 ->
     lc_tm b2 ->
      binds  F  ( sig_sort5 )   toplevel   ->
     Value R a ->
...
with reduction_in_one : tm -> tm -> role -> Prop :=    (* defn reduction_in_one *)
 ...
 | E_AppLeft : forall (a:tm) (nu5:nu) (b a':tm) (R1:role),
     lc_tm b ->
     reduction_in_one a a' R1 ->
     reduction_in_one (a_App a nu5 b) (a_App a' nu5 b) R1
...
with Typing : context -> tm -> tm -> Prop :=    (* defn Typing *)
 ...
 | E_Const : forall (G:context) (F:const) (A:tm) (Rs:roles),
     ...
     Typing  nil  A a_Star ->
     Typing G (a_Fam F) A
 | E_Fam : forall (G:context) (F:const) (A a:tm) (R1:role) (Rs:roles),
     Ctx G ->
      binds  F  ( (Ax a A R1 Rs) )   toplevel   ->
     Typing  nil  A a_Star ->
     Typing G (a_Fam F) A
 | E_Pat : forall (G:context) (R:role) (F:const) (a b1 b2 B:tm) (sig_sort5:sig_sort) (A:tm),
      binds  F  ( sig_sort5 )   toplevel   ->
     Typing G a A ->
     Typing G b1 B ->
     Typing G b2 B ->
     Typing G (a_Pattern R (a_Fam F) a b1 b2) B
 | E_case : forall (G:context) (a1 a2 b1 b2 B A:tm) (G':context) (R:role) (Rs:roles) (c:covar) (phi1:constraint),
     Typing G a1 A ->
     Pat G' a2 A R Rs ->
     Typing  ( G  ++   (  (( c ~ Co  phi1 ) ++  G' )  )  )  b1 B ->
     Typing G b2 B ->
      ( phi1  =   ( (Eq a1 a2 A R) )  )  ->
     Typing G (a_Pattern2 a1 a2 b1 b2) B
...
with DefEq : context -> available_props -> tm -> tm -> tm -> role -> Prop :=    (* defn DefEq *)
 ...
 | E_Right : forall (G:context) (D:available_props) (b b' A:tm) (R1 R':role) (F:const) (a:tm) (R:role) (Rs:roles) (a' B:tm) (R0:role),
     Path F a R'  ( R :: Rs )  ->
     Path F a' R'  ( R :: Rs )  ->
     Typing G a (a_Pi Rel A B) ->
     Typing G b A ->
...
Inductive Sig : sig -> Prop :=    (* defn Sig *)
 ...
 | Sig_ConsConst : forall (S:sig) (F:const) (A:tm) (Rs:roles),
     ...
     Sig  (( F ~ (Cs A Rs) )++ S ) 
 | Sig_ConsAx : forall (S:sig) (F:const) (a A:tm) (R:role) (Rs:roles),
     Sig S ->
     Typing  nil  a A ->
      ~ AtomSetImpl.In  F  (dom  S )  ->
     Sig  (( F ~ (Ax a A R Rs) )++ S ) .
...
(** infrastructure *)
Hint Constructors SubRole Path Pat Value value_type consistent roleing RhoCheck Par MultiPar joins Beta reduction_in_one reduction PropWff Typing Iso DefEq Ctx Sig lc_co lc_brs lc_tm lc_constraint lc_sort lc_sig_sort.
