...
Lemma ann_ctx_wff_mutual :
  (forall G0 a A R, AnnTyping G0 a A R -> AnnCtx G0) /\
  (forall G0 phi,   AnnPropWff G0 phi -> AnnCtx G0) /\
  (forall G0 D g p1 p2, AnnIso G0 D g p1 p2 -> AnnCtx G0) /\
  (forall G0 D g A B R,   AnnDefEq G0 D g A B R -> AnnCtx G0) /\
...
Ltac sort_inversion :=
  let h0 := fresh in
  match goal with
  | [ H : AnnTyping ([(?x,?s)] ++ ?G) _ _ _ |- _ ] =>
    have h0: AnnCtx (x ~ s ++ G); eauto with ctx_wff;
    inversion h0; subst; auto
  | [ H : AnnDefEq ([(?x,?s)] ++ ?G) _ _ _ _ |- _ ] =>
    have h0: AnnCtx (x ~ s ++ G); eauto with ctx_wff;
    inversion h0; subst; auto
...
Lemma lc_mutual :
  (forall G0 a A R, AnnTyping G0 a A R -> lc_tm a /\ lc_tm A) /\
  ...
  (forall G0 D g A B R,  AnnDefEq G0 D g A B R -> lc_tm A /\ lc_tm B /\ lc_co g) /\
...
Lemma AnnTyping_lc1 : forall G a A R, AnnTyping G a A R -> lc_tm a.
Proof.
  intros G a A R H. destruct (AnnTyping_lc H); auto.
Qed.
Lemma AnnTyping_lc2 : forall G a A R, AnnTyping G a A R -> lc_tm A.
  intros G a A R H. destruct (AnnTyping_lc H); auto.
Qed.
...
Proof. intros. destruct (AnnIso_lc H); split_hyp; auto. Qed.
Lemma AnnDefEq_lc1 : forall G D g A B R,  AnnDefEq G D g A B R -> lc_tm A.
Proof. intros. destruct (AnnDefEq_lc H); auto. Qed.
Lemma AnnDefEq_lc2 : forall G D g A B R,  AnnDefEq G D g A B R -> lc_tm B.
Proof. intros. destruct (AnnDefEq_lc H); split_hyp; auto. Qed.
Lemma AnnDefEq_lc3 : forall G D g A B R,  AnnDefEq G D g A B R -> lc_co g.
Proof. intros. destruct (AnnDefEq_lc H); split_hyp; auto. Qed.
