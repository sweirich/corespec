...
Lemma invert_a_App : forall G psi a psi0 b C,
    Typing G psi (a_App a psi0 b) C ->
    exists A B, Typing G psi a (a_Pi psi0 A B) /\
           CTyping G (psi0 * psi) b A /\
           DefEq (meet_ctx_l q_C G) q_C (Eq C (open_tm_wrt_tm B b) a_Star).
Proof.
  intros G psi a psi0 b C.
  move e : (a_App a psi0 b) => t1.
  induction 1; auto; try done.
  - exists A, B. inversion e; subst; repeat split; auto.
    apply Typing_regularity in H.
    apply invert_a_Pi in H.
    move : H => [h0 [[L h1] h2]].
    pick fresh x.
    spec x.
    apply Typing_tm_subst with (a := a0) in h1.
    apply E_Refl.
    rewrite (tm_subst_tm_tm_intro x).
    replace a_Star with (tm_subst_tm_tm a0 x a_Star); auto.
    fsetdec.
    apply CTyping_meet_ctx_l with (q := q_C) in H0.
    eapply CTyping_subsumption.
    eassumption.
    apply leq_meet_l.
  - destruct (IHTyping1 e) as [A0 [B0 [h3 [h2 e2]]]].
    exists A0, B0.
    repeat split; auto.
    apply (E_Trans _ _ _ _ _ A); auto.
Qed.

Lemma invert_a_CApp : forall G psi a g A,
    Typing G psi (a_CApp a g) A ->
    g = g_Triv /\
    exists phi B, Typing G psi a (a_CPi q_Top phi B) /\
             DefEq (meet_ctx_l q_C G) q_C phi /\
             DefEq (meet_ctx_l q_C G) q_C (Eq A (open_tm_wrt_co B g_Triv) a_Star).
Proof.
  intros G psi a g A H.
  dependent induction H.
  - sauto lq: on rew: off. 
  - split; first by done.
    exists phi, B1.
