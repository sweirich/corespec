...
Proof.
  ...
  - intros G1 x A R c H b D G2 H0 H1.
    ...
    eapply DefEq_weaken_available; eauto.
    eapply H; eauto.
  - intros L G1 rho A R B t H t0 H0 D G2 H1 H2.
...
Proof.
    ...
    + (have: Ctx G by eauto) => CTX.
      ...
      -- apply (E_PiCong (L \u (dom G))); auto.
         ++ intros x H4.
            ...
            apply Factor_Eqcontext_tm; eauto.
            apply refl_context_defeq; auto.
         ++ eapply E_SubRole. apply S.
            apply (E_Pi (L \u (dom G))); eauto.
            intros x H4.
            have: x `notin` L; auto => h0.
            destruct (H0 x h0).
            eapply Typing_regularity. eauto 2. admit.
         ++ eapply E_SubRole. apply S.
            apply (E_Pi (L \u (dom G))); eauto.
            intros x H4.
            eapply Typing_regularity; eauto 2. admit.
            (*apply H0; eauto. *)
      -- eapply E_SubRole. apply S. apply (E_Pi (L \u (dom G))); eauto.
         intros x H4.
         eapply Typing_regularity; eauto 2.
         admit. (* apply H0; eauto. *)
  ...
  - intros G D A A' a b R a' b' i H.
    ...
    auto. auto. auto. exact (dom G). exact (dom G). exact (dom G).
Admitted.
...
Lemma E_Pi2 : forall L G rho A B R,
